...
### 08 · A production-grade AGENTS.md template
*Distilled from 2,500+ real AGENTS.md files surveyed by GitHub.*

Generic Next.js + TypeScript + Postgres example — adapt the stack to yours, keep the structure. Note the three-tier **Boundaries** section (Ask first / Never) and the **When Stuck** escape hatch — both are common in well-run repos and rare in beginner templates.

```markdown
# AGENTS.md

## Project Overview
Customer-facing storefront. Next.js 15 (App Router) + TypeScript 5.4 +
Postgres 16 (Prisma) + Tailwind 3. Deployed to Vercel; staging on push to
`main`, prod on git tag `v*`. Business logic lives in `src/server/` — UI
components are dumb.

## Setup
- Node 20.x (`.nvmrc`), pnpm 9.x — never npm or yarn (lockfile is pnpm).
- `cp .env.example .env.local`, then load secrets from your secret manager
  (1Password CLI, Vault, etc.). Never paste real values into files.
- `pnpm install`
- `pnpm db:up` (starts Postgres in Docker), `pnpm db:migrate`, `pnpm db:seed`

## Common Commands
- Dev server: `pnpm dev` (http://localhost:3000)
- Type-check: `pnpm typecheck`
- Lint: `pnpm lint` · Format: `pnpm format`
- Unit tests: `pnpm test` (Vitest) — single file: `pnpm test src/lib/cart.test.ts`
- E2E tests: `pnpm e2e` (Playwright; requires `pnpm dev` running)
- New migration: `pnpm db:migrate:new <name>` — never edit existing migrations

## Repository Structure
- `src/app/` — App Router pages and route handlers (thin; delegate to `server/`)
- `src/components/` — presentational React components, no data fetching
- `src/server/` — business logic, db access (Prisma), external API clients
- `src/server/actions/` — server actions, all input-validated with Zod
- `src/lib/` — pure utilities, no I/O
- `prisma/` — schema + migrations (one migration per PR, max)
- `e2e/` — Playwright specs
- `.github/workflows/` — CI; mirror locally with `pnpm ci`

## Code Style
- Strict TS, no `any`, no non-null `!`. Prefer `unknown` + narrowing.
- Server-only modules: `import "server-only"` at top.
- Components: function declarations, props interface above the component.
- Database access only inside `src/server/`. UI never touches Prisma directly.
- Errors: throw `AppError` (see `src/lib/errors.ts`), never bare `Error`.
- Real example to mirror: `src/server/cart/add-item.ts`.

## Testing
- Unit tests next to source: `foo.ts` → `foo.test.ts`. Mock with `vi.mock`.
- Server actions need a Vitest test + a Playwright happy-path.
- New API route → add a test in `src/app/api/**/route.test.ts`.
- Don't add snapshot tests for components.

## Pull Requests
- Branches: `feat/`, `fix/`, `chore/`, `docs/` prefixes off `main`.
- Conventional commits (`feat(cart): ...`). One logical change per PR.
- PR body: what changed, why, screenshots for UI, migration notes if schema changed.
- CI must be green. Before pushing: `pnpm typecheck && pnpm lint && pnpm test`.
- Never force-push shared branches. Rebase, don't merge `main` in.

## Boundaries

**Ask first**
- Adding a new dependency (check `pnpm why` first; prefer existing).
- Schema changes that touch `users`, `orders`, `payments`.
- Anything that changes the public API contract in `src/app/api/`.
- Bumping framework majors (Next, React, Prisma).

**Never**
- Commit `.env*`, secrets, or values from your secret manager.
- Edit existing Prisma migrations — write a new one.
- Disable type-check, lint, or tests in CI to ship.
- Use `dangerouslySetInnerHTML` without a sanitizer + a comment explaining why.
- `git push --force` on `main` or release branches.

## When Stuck
- Build/test passes locally but fails in CI: check Node version, then `.env.example` drift.
- Prisma type errors after pulling: `pnpm db:generate`.
- Auth/session weirdness: see `src/server/auth/README.md`.
- Anything else: stop and summarize what you tried in the PR description.
```

> 💡 **Rules of thumb from real templates:**
> - **Front-load exact commands** — agents grep for `pnpm`/`uv`/`make` early. Put invocations with flags in the first third.
> - **Use the three-tier boundary section** — Always / Ask first / Never. The "Never" list stops the most expensive mistakes.
> - **Specifics beat prose** — "React 18 + TS 5.4 + Vite," not "a modern React project." One pointer to a real exemplar file beats three paragraphs on style.
> - **Give rules with rationale**, not just rules — agents follow rules they understand the reason for.
> - **Treat AGENTS.md as public** — point at where secrets live (1Password, Vault, `.env.example`), never the secrets themselves.

> ⚠️ **Do not stuff everything here:** detailed procedures belong in skills; temporary task context belongs in the prompt. Aim for under ~150 lines per repo; nest sub-AGENTS.md per package for larger codebases (the closest one wins).

**Deeper reading:**
- 📖 [How to write a great AGENTS.md — lessons from 2,500+ repositories](https://github.blog/ai-and-ml/github-copilot/how-to-write-a-great-agents-md-lessons-from-over-2500-repositories/) (GitHub Blog) — the single best reference.
- 🏆 Real production examples: [Apache Airflow](https://github.com/apache/airflow/blob/main/AGENTS.md) (gold standard), [OpenAI Codex](https://github.com/openai/codex/blob/main/AGENTS.md) (uses 88 nested AGENTS.md files), [Temporal Java SDK](https://github.com/temporalio/sdk-java/blob/master/AGENTS.md), [Roboflow Supervision](https://github.com/roboflow/supervision/blob/develop/AGENTS.md).
- 📚 Curated lists: [Ischca/awesome-agents-md](https://github.com/Ischca/awesome-agents-md), [tairov/awesome-agents.md](https://github.com/tairov/awesome-agents.md).
